...
const comfyUiAvailable = await checkComfyUi();
...
function requireComfyUI(res) {
  if (!comfyUiAvailable) {
    res.status(503).json({ error: "Free local AI backend is not running. Start ComfyUI on this PC and configure COMFYUI_URL/COMFYUI_WORKFLOW_JSON." });
    return false;
  }
  return true;
}
...
app.post("/api/generate", async (req, res) => {
  if (!requireComfyUI(res)) return;

  const { projectId, imagePath, prompt, ratio = "16:9", duration = 5 } = req.body || {};
  ...
  const jobId = uuid();
  updateJob(jobId, { id: jobId, status: "UPLOADING", progress: 5, projectId, provider: "comfyui-local", duration: numericDuration });
  ...
  (async () => {
    try {
      ...
      const buffer = await generateWithComfyUI({
        ...
        ratio,
        onProgress: progress => updateJob(jobId, { status: "GENERATING", progress })
      });
      ...
      updateJob(jobId, { status: "SUCCEEDED", progress: 100, videoPath: out });
    } catch (error) {
      updateJob(jobId, { status: "FAILED", progress: 100, error: error?.message || String(error) });
    }
  })();
});
